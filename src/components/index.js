...
function Index() {
  ...
  function pdfHistory(dateofbirth, dateofbirthTo){
    axios.get(`http://127.0.0.1:8000/history/withRange/${dateofbirth}/${dateofbirthTo}`)
      .then((res) => {
        setData(res.data);
      })
      .catch((error) => {
  ...
  const generatePDF = () => { 
    ...
    const dateofbirthTo = document.getElementById('dateofbirthTo').value;
    pdfHistory(dateofbirth, dateofbirthTo);
    const now = new Date();
    ...
    doc.text(`Generated by: ${generatedBy}`, 50, 30);
    doc.text(`From: ${dateofbirth}`, 50, 35);
    doc.text(`To: ${dateofbirthTo}`, 50, 40);       
    doc.autoTable({
    theme: 'grid',
